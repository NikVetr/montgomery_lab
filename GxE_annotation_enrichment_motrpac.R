...
#load GxE results
GxE <- fread("~/data/smontgom/gwas.chrALL.gxe.bmi.all.glm.linear.ADDxPA.hg38.txt")
rownames(GxE) <- GxE$V3
GxE$V1 <- gsub(GxE$V1, pattern = "_.*", replacement = "")
GxE <- split(GxE, GxE$V1)

SNP_frx <- fread("~/data/smontgom/ukbb.ALL.impute.frqx")
...
command_ldsc <- paste0("python2 munge_sumstats.py --sumstats ~/repos/ldsc/gwas_sumstats/", "BMI_GxE.txt.gz", 
