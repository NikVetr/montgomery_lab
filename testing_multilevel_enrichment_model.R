...
ncol <- 20
nrow <- 10
ncolhits <- sample(1:ceiling(n/5), size = ncol, replace = T)
nrowhits <- sample(ceiling(n/100):ceiling(n/10), size = nrow, replace = T)
correlated_cols <- T
col_nblocks <- 1
...
notbothhits <- sapply(1:ncol, function(ci) sapply(1:nrow, function(ri) length(intersect(colhits[[ci]], setdiff(1:n, rowhits[[ri]])))))

avg_row_prop_diff <- apply(diag(1/nrowhits) %*% bothhits - diag(1/(n-nrowhits)) %*% notbothhits, 1, mean)
...
corr_col_est <- estimate_correlations(setNames(colhits, 1:ncol), setNames(replicate(ncol, 1:n, F), 1:ncol), print_progress = T)
corr_row_est <- estimate_correlations(setNames(rowhits, 1:nrow), setNames(replicate(nrow, 1:n, F), 1:nrow))
...
d <- list(cell_count = data$count,
          ...
          row_n = nrow,
          col_n = ncol,
          L_row = t(chol(corr_row_est)),
          L_col = t(chol(corr_col_est)),
          L_int = t(chol(kronecker(corr_col_est, corr_row_est))))
...
data {
    ...
    cholesky_factor_corr[row_n * col_n] L_int;
...
transformed parameters {
    //recenter params
    vector[col_n] col_logodds = L_col * raw_col_logodds * col_sd;
    vector[n] cell_logodds = L_int * raw_cell_logodds * cell_sd + col_logodds[col_index];

    //incorporate difference
    vector[col_n] col_difference = L_col * raw_col_difference * col_difference_sd;
    vector[row_n] row_difference = L_row * raw_row_difference * row_difference_sd;
    vector[n] cell_difference = L_int * raw_cell_difference// * cell_difference_sd;
...
model {
    ...
    cell_count ~ binomial(row_count, Phi(cell_logodds_focal));
    cell_count_compl ~ binomial(row_count_compl, Phi(cell_logodds_compl));
...
out <- mod$sample(chains = 4, iter_sampling = 1E3, iter_warmup = 1E3, data = d, parallel_chains = 4,
                   adapt_delta = 0.85, refresh = 50, init = 0.1, max_treedepth = 15, thin = 2)
...
plot(avg_row_prop_diff, row_difference)
...
#reconstruct correlation matrices, if inferred
#columns
